...
function draw() {
  ...
  // If the ball has not been fired yet, define initial velocity
  if (!isFired) {
...
//CHECK COLLISIONS
  ...
  hasCollided=checkCircleRectCollision(position.x, position.y, ballSize/2, obstacles[i].x, obstacles[i].y, obstacles[i].w, obstacles[i].h, 0)
		if (hasCollided){
...
function drawFinalPositions(){
	for (let i = 0; i < finalPositions.length; i++) {
	  drawBall(finalPositions[i].xf,finalPositions[i].yf,color(255,204,84))
   }
}
